    ...
    private void findSegment(Point referencePoint, Point[] points) {
		...
		for (int i = 0; i < points.length; i++) {
			...
			if (areSlopesEqual(slope, currentSlope)) {
                ...
                // because we iterate through the points by they positions on the coordinates grid
			...
			if (i == points.length - 1 && nrOfPointsWithEqualSlope >= MIN_POINTS_NR_IN_SEGMENT && !isAlreadyFoundSegment) {
				this.addSegment(new LineSegment(referencePoint, prevPoint));
			}
